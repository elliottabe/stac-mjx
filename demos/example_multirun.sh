...
for version in "${VERSIONS[@]}"; do
    preproc_file="/data2/users/eabe/datasets/Johnson_lab/free_walking/${version}/preprocessed_bout.h5"
    if [ -f "$preproc_file" ]; then
        echo "  ✓ Found: $preproc_file"
    else
        echo "  ✗ Missing: $preproc_file"
        echo "    Run: cd $PREPROCESS_DIR && python scripts/preprocess_keypoints_for_ik.py paths=workstation dataset=free_walking version=$version"
    fi
done
echo ""
...
for version in "${VERSIONS[@]}"; do
    output_dir="/data2/users/eabe/datasets/Johnson_lab/free_walking/${version}/multirun_example"
    echo "  $output_dir"
done
echo ""
...
echo "  - free_walking_fruitfly_v1.mp4"
